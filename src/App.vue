...
onMounted(async () => {
  ...
  unlistenPerms = await onPermissionsStatus((status) => {
    ...
    if (
      ...
      console.warn(
        "Required permission lost — showing permissions guide",
        status,
      );
      currentView.value = "permissions";
    }
      ...
      <div class="relative">
        <div
          class="flex items-center justify-center w-12 h-12 rounded-xl
                 ...
                 text-gold-ink font-extrabold text-[16px]
                 shadow-glow-gold"
        >
          M
        </div>
        <div
          class="absolute -inset-2 rounded-2xl border-2 border-gold/25 animate-breathe"
        />
      </div>
      <p class="text-[11px] text-ink-muted font-medium tracking-tight">
        Starting Magpie…
      </p>
    </div>
