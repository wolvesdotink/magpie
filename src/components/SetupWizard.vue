...
onMounted(async () => {
  try {
    const [perms, availableModels, downloaded, userSettings] = await Promise.all([
      checkPermissions(),
      getAvailableModels(),
      getDownloadedModels(),
      getSettings(),
...
<template>
  <div class="flex flex-col h-full bg-canvas rounded-xl overflow-hidden">
    ...
    >
      ...
      <div class="relative">
        <div
          class="flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-gold via-gold to-gold-deep text-gold-ink shadow-glow-gold"
        >
          <svg
            class="w-9 h-9"
      ...
      <!-- CTA -->
      <button
        class="w-full max-w-[240px] py-2.5 rounded-lg text-[13px] font-semibold transition-all duration-200 active:scale-[0.97] bg-gradient-to-b from-gold to-gold-hover text-gold-ink hover:from-gold-hover hover:to-gold-deep shadow-press hover:shadow-lifted mt-2"
        @click="nextStep"
      >
        Get Started
      </button>
    ...
    >
      ...
      <div class="text-center">
        ...
        <p class="text-[10px] text-ink-muted leading-relaxed mt-1.5 max-w-[240px]">
          Magpie needs two permissions to work: microphone access for recording and accessibility
          for detecting the Fn key.
        </p>
      </div>
    ...
    <div v-else-if="currentStep === 'model'" class="flex flex-col flex-1 p-5 pt-6 min-h-0">
      <!-- Header -->
